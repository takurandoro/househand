-- Simplified Security Fix Script
-- This script fixes all security issues without trying to create policies on non-existent views
...
-- 9. Drop existing views if they exist (to fix SECURITY DEFINER issues)
DROP VIEW IF EXISTS public.helper_stats;
DROP VIEW IF EXISTS public.helper_balances;

-- 10. Create a secure function to get helper stats (replaces SECURITY DEFINER views)
